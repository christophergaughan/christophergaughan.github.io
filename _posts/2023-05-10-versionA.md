...
can be descibed as exploratory data analysis. EDA is used by data
...
**What shortcoming are there in using Jupyter Notebooks for harvesting
Clinical Data?**
...
7.  Reproducibility: Jupyter Notebooks can be challenging to reproduce,
    ...
    others or use it in a production environment. **Repeoducibility has
    been a very limitation during the creation of this notebook**
...
and point out potentisl weaknesses. These models don\'t hold much
...
and the actual relationship between these variables might not be
strong.**
...
y-values 600+ the rest of the values are closely aligned. The model
...
Polytherapies have to be a staple of cancer treatment. it seems unlikely
...
As you\'ll see, it doesn\'t work 100% because I just wanted antibodiess
on the x-axis.
:::
...
this feature to search a .csv file (essentially a small database)
:::
...
In summary, K-Nearest Neighbors has the highest accuracy of 0.60, while
...
20 samples, which may not be representative of the model\'s true
...
3.  Small dataset: With only 20 samples in the test set, it is possible
    that the dataset is not representative of the problem or that there
    is not enough data to accurately evaluate the model\'s performance.
    A larger dataset or a cross-validation approach could provide a more
...
-   Fitting 5 folds for each of 7600 candidates, totalling 38000 fits:
    ...
    cross-validation for each of the 7600 hyperparameter combinations,
    resulting in a total of 38,000 fits. The purpose of this process is
